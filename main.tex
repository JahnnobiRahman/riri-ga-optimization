...
only which category a prompt falls into. The evolved $\gamma$ parameter 
discovered in our experiments (Section~\ref{sec:results}) validates this 
gating mechanism as learnable and beneficial for safety.
...
$\theta_\mathrm{mid}$    & $[0.40,\,0.70]$ & Escalation threshold for mid-risk prompts; used in distress-gated escalation (Section~\ref{sec:risk-escalation}) \\
$\theta_\mathrm{high}$   & $[0.70,\,0.95]$ & Escalation threshold for high-risk prompts; reserved for future use (Section~\ref{sec:risk-escalation}) \\
$\gamma$ (distress gate)  & $[0.0,\,0.2]$ & Distress-gating parameter; relaxes $\theta_\mathrm{mid}$ for high-distress mid-risk prompts (Section~\ref{sec:risk-escalation}) \\
...
B4+$\gamma$ = GA with distress-score-gated escalation (improved);
B5 = GA + MSGA seeds;
...
B3: Random genome
  ...
  & --- \\
B4: GA Optimised (proposed)
  & 0.426 & 0.901 & 0.924 & 0.678 & --- \\
B4+$\gamma$: GA + Distress-Gated Escalation
  & \textbf{0.459} & \textbf{0.901} & 0.913 & \textbf{0.704} & --- \\
B5: GA + MSGA seeds
...
ratings; both are future work.
  ...
  escalation for high-distress mid-risk prompts. The GA evolves the $\gamma$ 
  parameter (converged: $\gamma \approx 0.093$) to balance when escalation 
  should trigger within a risk category. This extension improves safety 
  (+0.014) and empathy (+0.033) while maintaining structure completeness 
  (Section~\ref{sec:results}). Future work includes learning $h(x_i)$ from 
  clinician ratings rather than hand-tuned keywords.
