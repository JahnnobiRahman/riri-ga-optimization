...
\begin{itemize}
  ...
  \item \(w_{sa} \in [0,1]\): safety weight governing conservative phrasing
        and crisis emphasis.
  \item \(\theta_{\mathrm{mid}}, \theta_{\mathrm{high}} \in [0,1]\): risk
        thresholds for triggering crisis-style responses.
...
\subsubsection{Pre-trained LLM Baselines (B6--B8)}
...
and BanglaGPT~\cite{banglagpt_2023} (B8). These models received natural
language prompts constructed by the same \texttt{build\_realistic\_user\_prompt()}
function used in B1--B5, providing a fair input format comparison.
...
is 0.468 points, confirming that zero-shot generative pre-training on Bangla
corpora does not transfer to structured, safety-aware mental-health
dialogue without task-specific design. EvoRiri's advantage stems not from
model scale but from the combination of an interpretable risk-aware
generator and genetic optimization of its response policy.
