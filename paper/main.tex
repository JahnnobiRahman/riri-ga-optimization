...
\newcommand{\orcidauthorA}{0009-0002-7744-9853}
\newcommand{\orcidauthorB}{0000-0003-4963-8905}

\Author{Jahnnobi Rahman $^{1}$\orcidA{} and Mir Md.\ Jahangir Kabir $^{2,}$*\orcidB{}}\AuthorCitation{Rahman, J.; Kabir, M.M.J.}
\address[1]{Relaxy PTE. LTD., Singapore and Bangladesh; jahnnobirahman230@gmail.com}
...
automatic scores for empathy, safety, and structure, subject to strong per-example safety penalties
and smooth structure/empathy penalty terms. Experiments on a corpus of 2872 de-identified prompts with suicide-risk labels
...
users. The system composes responses via a structured, template-based
generator that controls when to include empathy, grounding, action
suggestions, and reflective questions, without calling an external LLM at
inference time (Section~\ref{sec:generator}). A
...
\begin{itemize}
  ...
  \item A trace-based analysis of evolutionary failure modes (structural
        ...
        and safety penalty terms yield behaviorally meaningful responses.
\end{itemize}
...
\textbf{H5:} Strong per-example safety penalties reduce unsafe or
inappropriate optimization shortcuts in high-risk mental-health contexts.
...
($w_e^{\star}=0.100$, $w_s^{\star}=0.229$, $w_c^{\star}=0.670$,
$\theta_{\mathrm{mid}}^{\star}=0.566$, $\theta_{\mathrm{high}}^{\star}=0.700$).
...
\begin{enumerate}
  ...
  \item \textbf{Generator.} Because $w_c^{\star}=0.670$ falls in the top
  ...
  \item \textbf{Fitness.} The response is below the 140-word penalty
        ...
        validation fitness $F(g^{\star}) \approx 0.677$
...
\subsubsection{Risk-Label Construction}
\label{sec:risk-labels}

Each prompt's risk label $r_i \in \{\mathrm{low}, \mathrm{mid}, \mathrm{high}\}$
is derived from two signals: the prompt's PHQ-4 total score and the presence
of explicit self-harm/suicide language in an accompanying clinician-authored
risk-assessment note recorded at the point of data collection. The PHQ-4 is
a validated four-item screening instrument for depression and anxiety
severity~\cite{phq4}, with standard severity bands of Normal (0--2), Mild
(3--5), Moderate (6--8), and Severe (9--12) (Appendix~\ref{app:dataset}).
PHQ-4 does not itself include a suicide or self-harm item, so it is not a
validated suicide-risk instrument on its own; we use its severity score as
one input to a risk label rather than as a direct measure of suicide risk.

The mapping from these two signals to the three-way risk label is our own
design choice, not a property of the PHQ-4 instrument itself:
\begin{equation}
  \label{eq:risk-label}
  r_i =
  \begin{cases}
    \mathrm{high} & \text{explicit risk indicator present, or PHQ-4 total} \ge 9 \\
    \mathrm{mid}  & \text{PHQ-4 total} \in [6, 8] \\
    \mathrm{low}  & \text{PHQ-4 total} \le 5
  \end{cases}
\end{equation}
The explicit risk indicator is detected via pattern matching against a fixed
set of English and Bangla phrases (e.g., ``suicide,'' ``kill myself,''
``hurt myself,'' ``end my life,'' ``unsafe,'' {\bengalifont আত্মহত্যা}) applied
to the risk-assessment note; a match escalates the label to \emph{high}
regardless of PHQ-4 score, since an explicit indicator is treated as taking
priority over screening-instrument severity. This escalation rule, and the
decision to retain explicit keyword detection as a parallel signal alongside
PHQ-4 severity, was proposed by the five clinical psychologists who later
served as evaluators in our human evaluation study
(Section~\ref{sec:human_eval}); the rationale given was that a PHQ-4 score
alone can understate acute risk in cases where a user discloses suicidal or
self-harm intent without this being reflected in elevated depression/anxiety
severity on the four PHQ-4 items.

We note two limitations of this labeling procedure that distinguish it from
a fully validated clinical risk-assessment protocol. First, while PHQ-4
itself is a validated instrument and the psychologists' input shaped the
labeling design, the specific score-to-label mapping in
Eq.~\eqref{eq:risk-label} and the exact keyword list have not themselves
been formally validated against clinician-assigned risk labels on a held-out
sample of this dataset (e.g., via inter-rater agreement or sensitivity/
specificity analysis). Second, the keyword-based override is a simple
pattern match and, like the empathy and safety markers used elsewhere in
this work (Section~\ref{sec:automatic-scoring}), cannot handle negation,
hypothetical phrasing, or context-dependent meaning. Because this override
is the component of the labeling pipeline most directly tied to
suicide/self-harm content, we treat it as the highest-priority target for
follow-up clinical validation, and we do not present the resulting risk
labels as equivalent in rigor to a clinician-assigned ground truth. We
return to this point in Section~\ref{sec:limitations}.
...
large enough. The converged genome \(w_c^{\star} = 0.670\)
...
Crisis-style escalation content is included if the prompt's risk label is high, 
\textit{or} if a continuous distress score $h(x_i)$ exceeds a learned threshold 
and the risk label is mid:
\begin{equation}
  \label{eq:risk-escalation}
    \text{escalate}(x_i, r_i, h_i) = \mathbb{I}\{ r_i = \mathrm{high} \} \vee 
    \mathbb{I}\{ r_i = \mathrm{mid} \land h_i > \tau_h - \gamma \cdot h_i \},
\end{equation}
where $h(x_i) \in [0,1]$ is a distress score computed from prompt keywords 
(suicide ideation, self-harm, hopelessness, isolation, severe overwhelm), 
$\tau_h = 0.65$ is the distress threshold, and $\gamma \in [0, 0.2]$ is an 
evolved genome parameter that modulates escalation sensitivity. Rearranging, 
escalation for a mid-risk prompt requires $h_i > \tau_h / (1+\gamma)$: 
increasing $\gamma$ lowers the effective distress threshold below $\tau_h$, 
allowing earlier escalation to crisis language without changing the formal 
risk label.

Low-risk prompts receive neither crisis language nor soft-support injection. 
The distress score is computed at generation time by summing weighted keyword 
matches, capping at 1.0, and applying a small penalty for marker stuffing 
(>3 distinct signals). This mechanism allows the GA to learn 
when escalation should trigger \textit{within} a risk category, rather than 
only which category a prompt falls into. The GA converged to a small evolved 
$\gamma$ ($\gamma^{\star} \approx 0.028$), indicating that only a modest 
relaxation of the fixed distress threshold $\tau_h$ was found beneficial; 
this suggests the fixed threshold already captures most of the useful 
escalation signal for this dataset, with the learned gating mechanism 
providing a secondary, fine-grained adjustment rather than a large 
behavioural shift.
...
\begin{align}
  \label{eq:per-example-fitness}
    f_i(g) = {}& A\,E_i + B\,S_i + C\,C_i - D\,L_i \notag\\
      ...
      & - 0.10\,\mathbb{I}\{h_i > \tau_h \wedge r_i=\mathrm{mid} \wedge \mathrm{esc}_i=0\},
\end{align}
...
A sixth term applies a direct fitness penalty of $-0.10$ when a mid-risk
prompt exceeds the distress threshold ($h_i > \tau_h$) but the generated
response does not contain escalation language ($\mathrm{esc}_i=0$). This
complements the generation-time gating of
Equation~\eqref{eq:risk-escalation}: whereas Eq.~\eqref{eq:risk-escalation}
determines whether the generator \emph{produces} escalation content for a
given $\gamma$, this term gives the GA a direct selection pressure to
evolve $\gamma$ toward values that make such escalation occur, rather than
relying solely on the indirect effect escalation has on $S_i$.
...
$\theta_\mathrm{mid}$    & $[0.40,\,0.70]$ & Reserved risk-escalation threshold gene; not currently read by the generator or fitness function (Section~\ref{sec:risk-escalation}) \\
$\theta_\mathrm{high}$   & $[0.70,\,0.95]$ & Reserved risk-escalation threshold gene; not currently read by the generator or fitness function (Section~\ref{sec:risk-escalation}) \\
$\gamma$ (distress gate)  & $[0.0,\,0.2]$ & Distress-gating parameter; relaxes the fixed distress threshold $\tau_h=0.65$ (not $\theta_\mathrm{mid}$) for high-distress mid-risk prompts (Section~\ref{sec:risk-escalation}) \\
$\lambda$ (length coef.) & $[0,\,1]$ & Part of the genome vector (Eq.~\eqref{eq:genome}) and subject to crossover/mutation, but not read by the generator or fitness function in the present implementation; the length penalty actually used in Eq.~\eqref{eq:per-example-fitness} is the fixed coefficient $D=0.05$ (see note below) \\
...
\emph{Note on $\lambda$:} like $\theta_\mathrm{mid}$ and $\theta_\mathrm{high}$,
$\lambda$ is defined in the genome (Eq.~\eqref{eq:genome}) as a reserved gene
for a future evolvable length-penalty coefficient, but the fitness function
implemented here (Eq.~\eqref{eq:per-example-fitness}) applies the length
penalty via the fixed constant $D$ rather than reading $\lambda$ from the
genome. Because $\lambda$ carries no selection gradient, it is expected to
drift under mutation rather than converge, in the same way as
$\theta_\mathrm{mid}$ and $\theta_\mathrm{high}$ (Appendix~\ref{app:genomes}).
We report it in the search-space table for completeness but omit it from the
reported best-genome values (Table~\ref{tab:hyperparams-ga}) since, like the
two threshold genes, its value is not operationally meaningful.
...
\hline
$w_e^{\star}$            & 0.100 & Empathy weight \\
$w_s^{\star}$            & 0.229 & Safety weight \\
$w_c^{\star}$            & 0.670 & Structure weight \\
$\theta_\mathrm{mid}^{\star}$ & 0.566 & Reserved gene, not currently functional (see note below) \\
$\theta_\mathrm{high}^{\star}$ & 0.700 & Reserved gene, not currently functional (see note below) \\
$\gamma^{\star}$         & 0.028 & Distress-gating parameter (evolved; see Section~\ref{sec:risk-escalation}) \\
$d^{\star}$              & 0     & Template index \\
Memory window$^{\star}$  & 512   & Context length (tokens) \\
\hline
\end{tabularx}
\end{table}
...
be interpreted as an optimised escalation policy. The same is true of the
genome's $\lambda$ gene, which is omitted from this table for that reason
(see the note following Table~\ref{tab:hyperparams-scoring}).

The best genome discovered by the GA ($w_c^{\star} = 0.670$) assigns
substantially higher weight to structural completeness than to empathy
($w_e^{\star} = 0.100$) or safety ($w_s^{\star} = 0.229$), with safety
receiving relatively more genome weight than empathy in the evolved policy.
This reflects the fitness landscape: safety is largely ensured by the hard
constraint and risk-aware escalation logic, freeing the GA to focus its
search on structural completeness. The result aligns with the large structure gains observed in
...
validation fitness remains essentially flat across all tested dataset sizes,
ranging narrowly from 0.676 ($N=2297$, the full training set) to 0.680
($N=1200$), a spread of only 0.004. Even the smallest dataset tested
($N=100$, mean fitness 0.677) is statistically indistinguishable from the
full-dataset result, and standard deviations across the five seeds per size
(0.002--0.006) overlap substantially across all five points. These results
support Hypothesis~H3 even more strongly than a rise-then-plateau pattern
would: rather than fitness improving up to a moderate dataset size before
saturating, the results show no measurable benefit from any increase in
dataset size within the range tested, indicating that the fitness signal
reaches its ceiling almost immediately for this generator and scoring
design.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/dataset_size_experiment_plot.png}
\caption{Effect of dataset size on validation fitness. Fitness is essentially
flat across all tested sizes, with overlapping error bars throughout,
indicating no measurable benefit from additional training data within this
range.}
...
Figure~\ref{fig:population-size} shows validation fitness across population
sizes ranging from $P=30$ to $P=400$. Unlike a rise-to-a-plateau pattern,
fitness fluctuates narrowly and non-monotonically across this range, from
0.676 ($P=200$) to 0.681 ($P=400$, the largest population tested), a spread
of only 0.005. The highest mean fitness occurs at the largest population
size rather than at an intermediate value, and standard deviations across
the five seeds per population size do not decrease monotonically with $P$
either (ranging from 0.0016 at $P=30$ to 0.0071 at $P=200$, the highest
value observed at any point in the sweep). Because the spread across all
six population sizes is comparable to, or smaller than, the run-to-run
standard deviation within a single population size, the error bars overlap
substantially across nearly every pair of adjacent points in
Figure~\ref{fig:population-size}. These results do not support
Hypothesis~H4 as originally framed: rather than an intermediate population
size offering a clear stability and performance advantage beyond which
returns diminish, fitness and variance both vary narrowly and
non-monotonically across the tested range, indicating that population size
has little systematic effect on outcome quality once $P$ is at least 30 for
this problem. Since larger populations incur substantially higher
computational cost with no corresponding fitness benefit, the practical
recommendation is to prefer smaller populations (e.g., $P=50$--$100$) for
computational efficiency.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/population_experiment_plot.png}
\caption{Population size sensitivity analysis. Fitness fluctuates narrowly
and non-monotonically across the tested range, with overlapping error bars
throughout, indicating no clear intermediate optimum.}
...
generated from the corrected 20-generation run with $P=100$, $T=20$, and
$|\mathcal{I}|=600$ (Section~\ref{sec:hyperparams}). The best fitness begins
at 0.680 in generation~1 and fluctuates within a narrow band of approximately
0.675--0.685 for the remainder of the run, with no sustained secondary
improvement phase; the single highest value observed (0.685, generation~14)
is only marginally above the initial value. This reflects the benefit of
seeded initialisation: the balanced seed genome $g_\mathrm{bal}$ already
occupies a high-fitness region, and the GA's mutation and crossover search
does not discover a substantially better configuration within 20
generations. Average fitness rises from 0.345 (generation~1) to
approximately 0.60 by generation~5, then oscillates between approximately
0.55 and 0.63 for the remainder of the run without a clear settling trend,
indicating that elite genomes consistently outperform the population mean
throughout optimisation, while population-level improvement plateaus earlier
than any further gain in the single best genome. Note that fitness values in
Figure~\ref{fig:fitness-curve} reflect training-time fitness including soft
constraint bonuses, which differs from the validation breakdown scores
reported in Table~\ref{tab:baseline-optimized}.
...
approximately 0.090 in generation~1, reflecting the mixed population of
seed genomes and random initialisations. It fluctuates between
approximately 0.022 and 0.090 across the run without a sustained downward
trend, indicating that ongoing mutation continuously reintroduces genetic
diversity and that elitism alone does not concentrate the population
strongly around a single configuration. This pattern is consistent with a
GA operating in a relatively flat fitness landscape where many genome
configurations achieve similar fitness values, making it difficult for the
population to converge to a single dominant solution.
...
B4+$\gamma$ = GA with distress-score-gated escalation;
B5 = GA + MSGA seeds;
...
\hline
B1: Zero-shot (no GA)
  & 0.296 & 0.445 & 0.020 & $-$0.203 & 0.031 \\
B2: Fixed $g_\mathrm{bal}$ (no GA)
  & 0.388 & 0.902 & 0.571 & 0.527 & 0.031 \\
B3: Random genome
  & $0.367$ \newline ${\pm\,0.092}$
  & $0.720$ \newline ${\pm\,0.228}$
  & $0.233$ \newline ${\pm\,0.263}$
  ...
  & --- \\
B4: GA Optimised (proposed)
  & 0.435 & 0.908 & 0.911 & 0.677 & --- \\
B4+$\gamma$: GA + Distress-Gated Escalation
  & 0.439 & 0.905 & 0.907 & 0.671 & --- \\
B5: GA + MSGA seeds
  & 0.439 & 0.903 & 0.924 & 0.683 & --- \\
\hline
...
a score of 0.911 compared to 0.020 for the zero-shot baseline, a 46-fold
increase. Safety is maintained at 0.908, closely matching B2 (0.902) and far
exceeding B1 (0.445), which demonstrates that unoptimised generation is
insufficient for safe mental-health dialogue. Empathy improves from 0.296
(B1) to 0.435 (B4), reflecting the GA's ability to balance empathetic
expression against structural and safety objectives simultaneously.

Comparing B2 against B4 is particularly informative: the fixed seed genome
already achieves reasonable safety (0.902) but poor structure (0.571) and
lower fitness (0.527). The GA search improves structure by 0.340 points and
overall fitness by 0.150 points beyond the seed genome, confirming that the
evolutionary search itself, and not just the seeded initialisation, is
responsible for the gains.

The high variance in B3 (fitness std $= 0.361$) confirms that random
parameter selection is unreliable for this task, with individual random genomes
ranging from negative fitness ($-0.227$) to near-optimal ($0.622$), reinforcing
...
The MSGA-seeded system achieves empathy 0.439, safety 0.903, structure 0.924,
and overall fitness 0.683, matching B4 (fitness 0.677) within run-to-run
...
($+0.004$) over B4 is within expected variance and should not be interpreted
...
language models evaluated in zero-shot mode on a fixed 400-prompt subsample
of the same 575-prompt validation set used for B1--B5 (drawn with the same
random seed, $\mathrm{seed}=7$): mT5-small~\cite{mt5_2021} (B6),
BanglaT5~\cite{banglat5_2022} (B7), and BanglaGPT~\cite{banglagpt_2023} (B8).
The subsample, rather than the full 575 prompts, was used to limit the
computational cost of beam-search decoding three separate models; because it
is drawn from the same validation split with the same seed rather than a
different or independently-drawn set, B6--B8 scores remain directly
comparable to B1--B5 on the same underlying prompt population. We frame
B6--B8 explicitly as a
...
The fitness gap between B4 (0.677) and the best pre-trained LLM (B7: 0.210)
is 0.467 points. We read this as characterising the gap between generic
...
\hline
1 & 0.6736 & $-$0.2150 & 0.5163 \\
2 & 0.6858 & $-$0.1743 & 0.5329 \\
3 & 0.6727 & $-$0.1916 & 0.5372 \\
4 & 0.6847 & $-$0.2061 & 0.5358 \\
5 & 0.6843 & $-$0.2083 & 0.5212 \\
\hline
Mean $\pm$ Std & $0.6802 \pm 0.0060$ & $-0.1991 \pm 0.0146$ & $0.5287 \pm 0.0085$ \\
\hline
...
Notably, the GA fitness is highly consistent across runs (std $= 0.006$),
indicating stable convergence properties. In contrast, B1 shows moderate
variability (std $= 0.015$) despite using a fixed genome, which reflects
...
indicating that the optimised genome ($w_c^{\star} = 0.670$) reliably
...
risk level or prompt content. Table~\ref{tab:baseline-optimized}'s
mean structure score for B4 (0.911) and B5 (0.924) is a validation-set
average of the discrete component-count lookup in
Eq.~\eqref{eq:structure}; it is not directly comparable to the
per-component usage frequency reported here, since a small number of
validation-set responses fall outside the deterministic $w_c \ge 0.55$
regime described in Section~\ref{sec:progressive-structure} and so do
not achieve the maximum $C_i = 1.00$, which is why the mean is slightly
below the 1.0 usage frequency shown in
Figure~\ref{fig:structure-usage}.
...
genome ($w_c^{\star} = 0.670$, $w_e^{\star} = 0.100$, $w_s^{\star} = 0.229$)
...
components, since $w_c^{\star} = 0.670$ falls in the top, deterministic
...
structure improvement (0.020 to 0.911), the safety gap (0.445 to
0.908), and the empathy improvement (0.296 to 0.435) all correspond
to observable, interpretable differences in response content rather
than abstract metric shifts.

The empathy gain is worth flagging on its own terms: a $+0.139$
absolute change (47\% relative) is modest next to the 46-fold
...
(0.296 to 0.435, 47\% relative), well behind the safety and structure
...
Comparing Tables~\ref{tab:baseline-optimized} and~\ref{tab:human_eval}, the
automatic empathy and safety scores rank B4 above B1 by the same ordering
as the corresponding human ratings, on both dimensions. The two scales are
not directly comparable in magnitude, but the consistent rank ordering
supports the automatic scores as useful optimisation signals even without
absolute calibration against clinical standards.
...
positive side, it accelerates convergence and improves reproducibility
across runs: because the population includes the four hand-designed seed
genomes alongside randomly initialised individuals, the best-fitness
trajectory in Figure~\ref{fig:fitness-curve} already starts near its final
range in generation~1 (best fitness 0.680, close to the run's maximum of
0.685), rather than requiring several generations of search to reach a
comparable region of the fitness landscape. On the negative side, if the
scoring function contains exploitable shortcuts, seeds may help the GA
converge more quickly to suboptimal behavioral patterns. In our
experiments, all seeds converged to question-only structural collapse
...
signal the fitness function provides, whether that signal is well-designed
or not. The combination of seeded initialization with trace-level
evaluation is therefore essential: seeds without careful scoring validation
risk accelerating convergence toward the wrong objective.
...
intervals across all larger dataset sizes. Mean runtime on our test hardware
(Figure~\ref{fig:runtime}) increases from 16.5 seconds at $N=100$ to a peak
of 124.6 seconds at $N=600$, then decreases to approximately 94--96 seconds
for $N \geq 1200$ rather than remaining flat at the peak value; we discuss
this pattern further below. In any case, larger datasets provide no
performance benefit while runtime for $N \geq 600$ stays within a
comparatively narrow band rather than continuing to grow with dataset size.
This has concrete practical implications: the
...
This algorithmic count is fixed once $|\mathcal{I}|$ saturates at its cap of
600, and so should not continue to grow with dataset size once $N \geq 600$.
Our measured wall-clock runtimes (Figure~\ref{fig:runtime}) are broadly
consistent with this in that runtime for $N \geq 600$ does not continue to
grow with dataset size, but the measurements do not show a clean plateau
either: runtime peaks at 124.6 seconds at $N=600$ and then decreases to
94--96 seconds for $N \geq 1200$, rather than remaining flat at the peak.
We attribute this non-monotonicity to system-level timing variability on
non-dedicated consumer hardware, e.g.\ background processes, thermal
throttling, and disk I/O contention across sequentially-run experiment
blocks, rather than to any property of the algorithm itself; the
$\mathcal{O}(P \cdot T \cdot |\mathcal{I}|)$ operation count, which is what
the complexity argument above actually establishes, is unaffected by this
wall-clock variability. This contrasts favourably with fine-tuning
...
\caption{Runtime scaling of GA optimisation with dataset size. Runtime rises
then falls rather than plateauing; see Section~\ref{sec:data-efficiency} for
...
search. The balanced seed genome already achieves high safety (0.902) but
leaves substantial room for structural improvement (0.571 vs.\ 0.911). The GA
discovers that high structure weight ($w_c \approx 0.67$--$0.74$) consistently
...
These observations align with Hypothesis~H5: the safety penalty and
risk-aware escalation logic prevent the GA from exploiting structural
shortcuts at the cost of safety, as evidenced by B4 maintaining safety
parity with B2 (0.908 vs.\ 0.902) while dramatically improving structure.
The same comparison supports Hypothesis~H2: progressive structure
enforcement drives the 46-fold structural completeness gain over B1
(and the 0.340-point gain over B2) without substantially reducing safety,
which stays essentially flat between B2 and B4.

The absence of a clear population-size effect (Section~\ref{sec:results},
population size sensitivity) is itself informative rather than merely a
non-result. For practitioners considering similar template-based,
GA-optimized generators, it suggests that computational budget is better
spent on other hyperparameters, or on dataset and scoring-function design,
rather than on scaling population size beyond a modest value, at least for
problems with a comparably small, low-dimensional genome such as the one
used here. A well-evidenced absence of effect, with overlapping error bars
and non-monotonic variance across the tested range as reported above, is a
useful and generalisable finding in its own right, distinct from a failure
to find an effect that may simply reflect an underpowered experiment.
...
$w_c^{\star}=0.670$, versus $w_e^{\star}=0.100$ and $w_s^{\star}=0.229$
...
third? Our results (Section~\ref{sec:interp-baseline}) are more consistent
with the latter: safety and empathy stay near their B2 levels while structure
climbs substantially. The GA's contribution is concentrated in the one
objective with available headroom rather than a smooth trade-off across all
three. This is a genuine limitation of collapsing
...
behaviour is also therapeutically optimal behaviour at the margin: a genome
the metric ranks highest may not be the genome a clinician would rank
highest, particularly in borderline cases the marker lists do not
anticipate. The human evaluation (Section~\ref{sec:human_eval}) supports the
proxies as directionally valid but, as we discuss in full in
Section~\ref{sec:limitations}, does not establish calibration against
clinical judgement in absolute terms or across the full genome space. Until
that gap is closed, we treat the proxies as an interpretable but unvalidated
approximation, and we recommend that any deployment include ongoing
monitoring by qualified mental health professionals who can identify cases
where high-scoring responses are clinically inappropriate.
...
given that the optimised genome's high structure weight ($w_c^{\star} = 0.670$)
...
ratings; both are future work.
  ...
  \item \textbf{Distress-score-gated escalation implemented.} The present 
  implementation now includes a continuous distress score $h(x_i)$ computed 
  from prompt keywords (Section~\ref{sec:risk-escalation}), which gates 
  escalation for high-distress mid-risk prompts. The GA converges to a small 
  evolved $\gamma$ ($\gamma^{\star} \approx 0.028$), and the resulting 
  distress-gated system (B4+$\gamma$ in Table~\ref{tab:baseline-optimized}) 
  performs comparably to the base GA-optimised system B4 (safety $-0.003$, 
  empathy $+0.004$, structure $-0.004$, fitness $-0.006$), indicating that 
  the fixed threshold $\tau_h=0.65$ already captures most of the useful 
  escalation signal for this dataset, with the learned gating mechanism 
  providing only a marginal, largely neutral adjustment rather than a 
  substantial improvement. Future work includes learning $h(x_i)$ from 
  clinician ratings rather than hand-tuned keywords, and testing whether 
  $\gamma$'s effect becomes more pronounced on prompts nearer the distress 
  threshold.
  ...
  emotional context accumulates. Recent work argues that the basic
  unit of safety evaluation in mental-health chatbots should be the
  dialogue rather than the single turn, since risk-relevant cues
  accumulate across a conversation rather than appearing at one fixed
  point~\cite{journey2026}; empirical evidence confirms that
  concerning chatbot behaviour escalates significantly across
  conversation turns, with the trajectory varying by user
  vulnerability profile~\cite{vail2026}. The current genome
  parametrises a single response policy; extending to multi-turn
  would require a sequential decision framework where the GA
  optimises a turn-level policy conditioned on conversation history.
  Preliminary exploratory work in this direction surfaced three
  findings relevant to future study. First, escalation \emph{timing}
  can be made conversation-history-aware without requiring the
  underlying response generator to reference prior turns---suggesting
  these are separable concerns that can be tackled independently.
  Second, a template-based generator's phrase banks, adequate for
  independent single-turn evaluation, require substantial expansion
  to sustain variety across extended multi-turn dialogue. Third, and
  more precisely: analysis of the evolved $\gamma$ parameter across
  36{,}113 real mid-risk conversation turns revealed that $\gamma$'s
  practical effect on escalation decisions is a step function rather
  than a smooth relationship. Below a threshold of approximately
  $\gamma \approx 0.086$, changing $\gamma$ affects zero turns;
  above it, 869 turns (2.41\% of the dataset) flip simultaneously,
  after which $\gamma = 0.10$, $0.15$, and $0.20$ produce identical
  escalation behaviour. The threshold corresponds precisely to where
  the effective relaxed threshold $\tau_h - \gamma h_i$ crosses
  below the dominant distress-score cluster at $h_i = 0.60$,
  confirming that $\gamma$'s ceiling-seeking behaviour in multi-turn
  runs reflects fitness-neutral drift above a real, identifiable
  saturation point---the same phenomenon documented for
  $\theta_\mathrm{mid}$/$\theta_\mathrm{high}$ in
  Appendix~\ref{app:genomes}, here characterised with a
  specific quantitative explanation. The \texttt{memory\_window} gene
  in the current genome was designed with this extension in mind and
  provides a natural starting point for future work.
...
full conversation history---the most impactful near-term direction,
supported both by clinical literature~\cite{journey2026,vail2026} and
by the preliminary architectural findings noted above; (2)~multi-objective evolutionary algorithms maintaining Pareto
...
Intermediate (Gen 10)
  & 0.101 & 0.163 & 0.736 & 0.473 & 0.921 & 0.681 \\
Converged best (Gen 14)
  & 0.100 & 0.229 & 0.670 & 0.566 & 0.700 & 0.685 \\
\hline
\end{tabularx}
\end{table}

The evolution shows a clear directional shift: $w_c$ increases from 0.340
to 0.670 across the run, while $w_e$ decreases toward its minimum floor
value; $w_s$ settles at a moderate value above its floor rather than at the
floor itself. This reflects the fitness landscape discussed in
...
(0.550$\to$0.473$\to$0.566 and 0.800$\to$0.921$\to$0.700 respectively): they
...
here. The genome's $\lambda$ gene (Section~\ref{sec:hyperparams}) is
expected to behave identically for the same reason, though it is omitted
from this table since it was not tracked separately from $\theta_\mathrm{mid}$
and $\theta_\mathrm{high}$ in these logged snapshots.
