...
evolved genome parameter that modulates escalation sensitivity. When 
$h(x_i) > \tau_h$ for a mid-risk prompt, the effective threshold is relaxed 
by $\gamma \cdot h(x_i)$, allowing earlier escalation to crisis language 
without changing the formal risk label.
...
increases from approximately 16.5 seconds at $N=100$ to a peak of 124.6
seconds at $N=600$, then decreases to approximately 94--96 seconds for
$N \geq 1200$ rather than remaining flat, a pattern we attribute to
system-level timing variability on non-dedicated hardware rather than to
the algorithm's computational complexity, which is fixed once the
evaluation subset saturates at $|\mathcal{I}|=600$.}
\label{fig:runtime}
\end{figure}
...
by B4 maintaining safety parity with B2 (0.908 vs.\ 0.902) while dramatically
...
baseline B2 (safety 0.908 vs.\ 0.902; Section~\ref{sec:interp-baseline})
while structure improves by 0.340 points beyond B2. The GA's contribution
...
ratings; both are future work.
  ...
  Preliminary exploratory work in this direction surfaced two
  architectural considerations for future study: first, that
  escalation \emph{timing} can be made conversation-history-aware
  without requiring the underlying response generator to reference
  prior turns---suggesting these are separable concerns that can be
  tackled independently; and second, that a template-based
  generator's phrase banks, adequate for independent single-turn
  evaluation, require substantial expansion to sustain variety across
  extended multi-turn dialogue. The \texttt{memory\_window} gene in
  the current genome was designed with this extension in mind and
